...
- [ ] Stage A delivered: wicket-orientdb, transponder-orientdb, wicket-console, logger resolvable from a real repo (Central or Central Portal snapshots)
  ...
  - transponder-orientdb, wicket-console, logger: Stage A started 2026-09-25, each in its own opencode/tmux session with the same prompt plus phase R prep; wicket-orientdb is the reference implementation
    - Transponder: plan approved. JUnit 5.14.4; ByteBuddy 1.15.10 → 1.18.x (1.15 refuses Java 25); OrientDB 3.2.56 with the GraalJS swap in **test** scope only (`orientdb-core` is `provided` there); unused drivers (arcadedb, neo4j, janusgraph, mongodb) fixed within their DB major or parked. Stage B/C need no work (no javax/Wicket)
    - logger: plan approved. Tooling-only Stage A; also fixes `DefaultOLoggerEventFactory` ignoring `source`/`metaData` (unreleased API). Stage B/C need no work; its `toJson()` shape and class names are frozen (Orienteer parses and stores them)
    - wicket-console: analysis in progress
    - Namespace `org.orienteer` (Transponder, logger): verification and SNAPSHOTs on the Central Portal unconfirmed. Owner must confirm before their snapshot deploys
