...
- [ ] Stage A delivered: wicket-orientdb, transponder-orientdb, wicket-console, logger resolvable from a real repo (Central or Central Portal snapshots). Status 2026-09-25: Stage A done for all four; wicket-orientdb published, the other three installed locally only
  - wicket-orientdb: **Stage A done locally** (2026-09-24; pushed 2026-09-25, CI green on JDK 21/25; `2.0-SNAPSHOT` published to the Central Portal). Installed in `~/.m2`: parent pom, jar and test-jar `2.0-SNAPSHOT`, Java 21 bytecode. Built against Wicket 8.15, OrientDB 3.2.56, GraalJS 25.0.4, OkHttp 4.9.0, Lombok 1.18.48. 103 tests run, 0 failures, 17 skipped (pre-existing `@Ignore`) on JDK 21 and 25; the JDK 25 run was re-checked independently. No public API changes. Also fixes empty query-backed data providers on OrientDB 3.2.56 (`OQueryModel.iterator`). Full handoff: the "Handoff to Orienteer" section of its `REFRESH_PLAN.md`; the consequences for Orienteer are D8 and P3. Still to do: publishing it (its phase R)
  - wicket-orientdb decisions (owner, 2026-09-24): GraalJS swap accepted; versioning per D9; demo module kept after Stage C; stale remote branches kept for now. Phase R prepared (2026-09-24; pushed 2026-09-25): GitHub Actions CI (JDK 21/25), `central-publishing-maven-plugin` 0.11.0 (server `central`, manual Publish in the portal), release profile (sources, javadoc, GPG), Dependabot, JaCoCo; build green on 21/25. Snapshot deployed 2026-09-24 (token stored under server id `ossrh`: `-Dcentral.server.id=ossrh`). Still needed for the `2.0` release: a GPG key on a keyserver, and Orienteer P3 green against the published snapshots. Runbook: the library's `REFRESH_PLAN.md`
  - transponder-orientdb, wicket-console, logger: Stage A started 2026-09-25, each in its own opencode/tmux session with the same prompt plus phase R prep; wicket-orientdb is the reference implementation
    - Transponder: **Stage A done locally, phase R prepared** (2026-09-25; pushed 2026-09-25, CI green on JDK 21/25; snapshot not published yet). Installed in `~/.m2` as `1.1-SNAPSHOT`: parent, core (+ test-jar), orientdb, arcadedb, mongodb; Java 21 bytecode. ByteBuddy 1.18.14, OrientDB 3.2.56 (`provided`), GraalJS swap in test scope only. 112 tests green on JDK 21 and 25 (re-checked independently on 25). No public API changes (`javap` diff); one behavior fix: `sealed` `@EntityType` interfaces now work. No JVM flags. Parked (not published): neo4j (embedded 4.4 can't start on JDK 21+) and janusgraph (6/20 tests fail as before). Stage B/C: nothing to do. Owner decision pending: publish arcadedb/mongodb too (default: yes)
    - logger: **Stage A done locally, phase R prepared** (2026-09-25; pushed 2026-09-25, CI green on JDK 21/25; snapshot not published yet). `org.orienteer:logger:1.4-SNAPSHOT` installed in `~/.m2`: jar + pom, Java 21 bytecode, no runtime dependencies. 4 tests green on JDK 21 and 25 (re-checked independently on 25). No public API changes (compared with `javap`), and `toJson()` is unchanged. Behavior fix: `DefaultOLoggerEventFactory` now applies `source`/metadata (Orienteer only calls `OLogger.log(ex)`, which is unaffected). No JVM flags. Stage B/C: nothing to do, so Orienteer keeps 1.4 through P5/P6. Open points for Orienteer: correlation IDs differ per JDK (see P8 logger-server); the default dispatcher sends synchronously with no connect/read timeout, so an unreachable collector blocks the request thread (P8/P10)
    - wicket-console: **Stage A done locally, phase R prepared** (2026-09-25; pushed 2026-09-25, CI green on JDK 21/25; snapshot not published yet). Installed as `1.4-SNAPSHOT` (parent + jar), Wicket 8.3 → 8.15, Java 21 bytecode, no script engine or logging binding in the library (enforced). 7 library + 4 demo tests green on JDK 21 and 25 (re-checked on 25). No public API changes. Behavior fixes: missing engine → error result instead of NPE; default-engine fallback; GraalJS state kept between commands (was a new context per command, and a leak); GraalJS scripts may call Java again as with Nashorn, scoped to the console's own engines (its D13; IO/threads/native stay off). Stage B ≈ 0.5 day (`1.5-SNAPSHOT`), Stage C ≈ 0.5–1 day (`2.0-SNAPSHOT`). Known bugs for its 1.4 or 1.5: first-request history lost, non-thread-safe session cache, GraalJS contexts never closed
    - GitHub (2026-09-25): Orienteer and all four libraries pushed. wicket-orientdb's `master` has a "changes must be made through a pull request" rule, which the owner account bypassed. Dependabot is now active in the libraries and opened PRs (wicket-orientdb 3 new, Transponder 8, wicket-console 2), including Wicket 8.15 → 8.19 and GraalJS 25.0.4 → 25.4.x (see P4). Orienteer's CI fails only on the missing Transponder snapshot, as expected
    - Snapshot deploys waiting for the owner (all prepared, command `./mvnw -B clean deploy -Prelease -Dgpg.skip -Dcentral.server.id=ossrh` from a clean `master` on JDK 21, after pushing): wicket-console (`ru.ydn`, verified) can go now; Transponder and logger need the `org.orienteer` namespace verified with SNAPSHOTs enabled first
...
- [x] Make the external libraries resolvable (outside this repo, D9): wicket-orientdb 2.0-SNAPSHOT (done 2026-09-24: Stage A, published to the Central Portal snapshots), transponder-orientdb 1.1-SNAPSHOT, wicket-console 1.4-SNAPSHOT, logger 1.4-SNAPSHOT — 2026-09-25: all four resolvable on this machine (Stage A installed locally); only wicket-orientdb is published so far, the other three wait for the owner's snapshot deploys
...
- [x] Exit check: `./mvnw -fn dependency:tree` reports no missing POM for any non-parked module. — 2026-09-25: **0 missing POMs**, all 18 projects resolve (offline, from `~/.m2`). Earlier status: Status 2026-09-25: all 18 reactor projects pass `validate`; `dependency:tree` reports exactly 3 missing POMs: `transponder-orientdb:1.1-SNAPSHOT`, `wicket-console:1.4-SNAPSHOT`, `logger:1.4-SNAPSHOT`. `dependency:go-offline` / `resolve-plugins` can't run until P2 removes cobertura 2.7, which needs JDK 8's `tools.jar`
...
- [ ] Wicket 8.15 → **8.19.0** (latest 8.x, patch releases in the same major, found by Dependabot) in lockstep with wicket-orientdb and wicket-console, before P5
- [ ] GraalJS 25.0.4 → latest 25.x (25.4.x seen; Dependabot PRs open in the libraries) in lockstep with wicket-orientdb and Transponder; re-run `ServerSideJavaScriptTest`
- [ ] Close Orienteer's obsolete PRs from 2020–2023 (old Dependabot bumps #830–#841, all superseded by P2/P3; community PRs #641, #778 need a decision) — owner
...
- [ ] **devutils / logger-server:** follow external Stage A–C; no local work beyond jakarta renames. Note for logger-server: exception correlation IDs hash `StackTraceElement.toString()`, which changed after JDK 8 (module names, line numbers), so incidents recorded after the JDK 21 switch won't group with older ones. Document it, or normalize the hash input
...
- [ ] logger client (`DefaultOLoggerEventDispatcher`) sends incidents synchronously with no connect/read timeout; set timeouts or dispatch asynchronously so an unreachable collector can't block requests (found in logger Stage A)
