 ...
 * SELECT FROM Class WHERE embedded.keys() CONTAINS :value
 * @param <T> type of value
 */
public class EmbeddedContainsKeyPanel<T extends Serializable> extends EqualsFilterPanel<T> {
    public EmbeddedContainsKeyPanel(String id, IModel<T> model, String filterId, IModel<OProperty> propertyModel, IVisualizer visualizer, IFilterCriteriaManager manager) {
    ...
    protected void setFilterCriteria(IFilterCriteriaManager manager, FilterCriteriaType type, IModel<T> filterModel) {
        manager.addFilterCriteria(manager.createEmbeddedContainsKeyCriteria(filterModel, getJoinModel()));
    }
