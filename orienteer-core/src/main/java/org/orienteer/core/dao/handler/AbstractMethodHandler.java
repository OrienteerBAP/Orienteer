...
public abstract class AbstractMethodHandler<T> implements IMethodHandler<T> {
  ...
  protected static ResultHolder returnChained(Object proxy, Method method, boolean present) {
    ...
    return present ? returnChained(proxy, method) : NULL_RESULT;
  }
  ...
  protected static Object prepareForDB(Object arg) {
    ...
    if (OType.isSimpleType(arg)) {
    	return arg;
		} else if (arg instanceof IODocumentWrapper) {
    	return prepareForDB(((IODocumentWrapper) arg).getDocument());
		} else if (arg instanceof ODocumentWrapper) {
    	return prepareForDB(((ODocumentWrapper) arg).getDocument());
    ...
    throw new IllegalStateException("Type " + arg.getClass() + " can't be cast to use in DB");
  }
